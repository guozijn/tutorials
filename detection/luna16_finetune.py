...
def load_pretrained_weights(net, pretrained_model_path):
    if not pretrained_model_path:
        print("No pretrained model was provided; fine-tuning will start from random initialization.")
        return

    pretrained = torch.jit.load(pretrained_model_path, map_location="cpu")
    incompatible = net.load_state_dict(pretrained.state_dict(), strict=False)
    print(f"Loaded pretrained weights from {pretrained_model_path}")
    if incompatible.missing_keys:
        print(f"Missing keys when loading pretrained weights: {incompatible.missing_keys}")
    if incompatible.unexpected_keys:
        print(f"Unexpected keys when loading pretrained weights: {incompatible.unexpected_keys}")
    del pretrained


def set_backbone_trainable(net, trainable):
    backbone_keywords = [
        "feature_extractor.body",
        "body.conv1",
        "body.bn1",
        "body.layer1",
        "body.layer2",
    ...
    for name, param in net.named_parameters():
        is_backbone = any(keyword in name for keyword in backbone_keywords)
        if is_backbone:
...
def build_optimizer(net, lr):
    ...
    return torch.optim.SGD(trainable_params, lr, momentum=0.9, weight_decay=3e-5, nesterov=True)


def main():
    parser = argparse.ArgumentParser(description="PyTorch Object Detection Fine-tuning")
    parser.add_argument(
        "-e",
        "--environment-file",
    ...
    parser.add_argument(
        ...
        "--pretrained-model",
        default=None,
        help="pretrained TorchScript RetinaNet model path, e.g. ./trained_models/model_luna16_fold1.pt",
    )
    parser.add_argument(
        "--max-epochs",
        type=int,
        default=None,
        help="fine-tuning epochs. Defaults to config fine_tune_max_epochs or 100.",
    )
    parser.add_argument(
        "--head-only-epochs",
        type=int,
        default=None,
        help="epochs to train only detection heads before unfreezing the full network. Defaults to config head_only_epochs or 20.",
    )
    parser.add_argument(
        "--finetune-lr",
        type=float,
        default=None,
        help="fine-tuning learning rate. Defaults to config fine_tune_lr or 1e-4.",
    )
    parser.add_argument(
        "--head-lr",
        type=float,
        default=None,
        help="learning rate for the head-only warmup stage. Defaults to 10x finetune lr.",
    )
    parser.add_argument(
        "--val-interval",
        type=int,
        default=None,
        help="validation interval in epochs. Defaults to config val_interval or 5.",
    )
    ...
    try:
        explicit_val_data = load_decathlon_datalist(
            args.data_list_file_path,
            is_segmentation=True,
    ...
    if explicit_val_data:
        train_items = train_data
        val_items = explicit_val_data
        print(f"Using explicit datalist split: {len(train_items)} training, {len(val_items)} validation.")
    else:
        split_idx = int(0.95 * len(train_data))
        train_items = train_data[:split_idx]
        val_items = train_data[split_idx:]
        print(f"No explicit validation split found; using 95/5 split: {len(train_items)} training, {len(val_items)} validation.")
    ...
    pretrained_model_path = args.pretrained_model or env_dict.get("pretrained_model_path")
    load_pretrained_weights(net, pretrained_model_path)
    ...
    # 4. Initialize fine-tuning
    fine_tune_lr = args.finetune_lr or config_dict.get("fine_tune_lr", 1e-4)
    head_lr = args.head_lr or config_dict.get("head_lr", fine_tune_lr * 10.0)
    head_only_epochs = args.head_only_epochs
    if head_only_epochs is None:
        head_only_epochs = config_dict.get("head_only_epochs", 20)
    max_epochs = args.max_epochs or config_dict.get("fine_tune_max_epochs", 100)
    val_interval = args.val_interval or config_dict.get("val_interval", 5)

    if head_only_epochs > 0:
        print(f"Stage 1: freeze backbone and train detection heads for {head_only_epochs} epochs.")
        set_backbone_trainable(detector.network, trainable=False)
        optimizer = build_optimizer(detector.network, head_lr)
    else:
        print("Stage 1 disabled: train the full network from epoch 1.")
        set_backbone_trainable(detector.network, trainable=True)
        optimizer = build_optimizer(detector.network, fine_tune_lr)

    after_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=max(1, max_epochs // 2), gamma=0.1)
    ...
    for epoch in range(max_epochs):
        if head_only_epochs > 0 and epoch == head_only_epochs:
            print(f"Stage 2: unfreeze full network and continue fine-tuning at lr={fine_tune_lr}.")
            set_backbone_trainable(detector.network, trainable=True)
            optimizer = build_optimizer(detector.network, fine_tune_lr)
            after_scheduler = torch.optim.lr_scheduler.StepLR(
                optimizer,
                step_size=max(1, (max_epochs - head_only_epochs) // 2),
                gamma=0.1,
            )
